...
public interface ModelCacheFactory {
	...
	default List<QuadData> buildQuads(UVUnion pUvUnion, VertexSet pVertices, Cube pCube, float pTextureWidth, float pTextureHeight, boolean pMirror) {
		...
		quads.set(0, buildQuad(pVertices, pCube, pUvUnion, pTextureWidth, pTextureHeight, pMirror, Direction.WEST));
		quads.set(1, buildQuad(pVertices, pCube, pUvUnion, pTextureWidth, pTextureHeight, pMirror, Direction.EAST));
		quads.set(2, buildQuad(pVertices, pCube, pUvUnion, pTextureWidth, pTextureHeight, pMirror, Direction.NORTH));
		quads.set(3, buildQuad(pVertices, pCube, pUvUnion, pTextureWidth, pTextureHeight, pMirror, Direction.SOUTH));
		quads.set(4, buildQuad(pVertices, pCube, pUvUnion, pTextureWidth, pTextureHeight, pMirror, Direction.UP));
		quads.set(5, buildQuad(pVertices, pCube, pUvUnion, pTextureWidth, pTextureHeight, pMirror, Direction.DOWN));
	...
	record VertexSet(VertexData bottomLeftBack, VertexData bottomRightBack, VertexData topLeftBack,
		...
		public List<VertexData> verticesForQuad(Direction pDirection, boolean pBoxUv, boolean pMirror) {
			return switch (pDirection) {
				case WEST -> pMirror ? quadEast() : quadWest();
				case EAST -> pMirror ? quadWest() : quadEast();
				case NORTH -> quadNorth();
				case SOUTH -> quadSouth();
				case UP -> pMirror && !pBoxUv ? quadDown() : quadUp();
				case DOWN -> pMirror && !pBoxUv ? quadUp() : quadDown();
			};
		}
	}
}
