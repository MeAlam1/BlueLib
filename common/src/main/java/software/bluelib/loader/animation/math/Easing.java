...
public interface Easing {
	...
	static double linear(double pN) {
		return pN;
	}

	static double quadratic(double pN) {
		return pN * pN;
	}

	static double cubic(double pN) {
		return pN * pN * pN;
	}

	static double sine(double pN) {
		return 1 - Math.cos(pN * Math.PI / 2f);
	}

	static double circle(double pN) {
		return 1 - Math.sqrt(1 - pN * pN);
	}

	static double exp(double pN) {
		return Math.pow(2, 10 * (pN - 1));
	}

	// ---> Easing Curve Functions <--- //

	static Double2DoubleFunction elastic(Double pN) {
		double n2 = pN == null ? 1 : pN;

		return t -> 1 - Math.pow(Math.cos(t * Math.PI / 2f), 3) * Math.cos(t * n2 * Math.PI);
	}

	static Double2DoubleFunction bounce(Double pN) {
		final double n2 = pN == null ? 0.5d : pN;
	...
	static Double2DoubleFunction back(Double pN) {
		final double n2 = pN == null ? 1.70158d : pN * 1.70158d;

		return t -> t * t * ((n2 + 1) * t - n2);
	}

	static Double2DoubleFunction pow(Double pN) {
		return t -> Math.pow(t, pN);
	}

	static Double2DoubleFunction step(Double pN) {
		double n2 = pN == null ? 2 : pN;
