...
public record PolysUnionDeserializer(
		@NotNull List<List<List<Float>>> union,
		@Nullable Type type) {

	@NotNull
	public static JsonDeserializer<PolysUnionDeserializer> deserializer() throws JsonParseException {
		return (json, type, context) -> fromJson(json, context);
	}

	@NotNull
	public static PolysUnionDeserializer fromJson(@NotNull JsonElement pJson, @NotNull JsonDeserializationContext pContext) throws JsonParseException {
		if (pJson.isJsonPrimitive() && pJson.getAsJsonPrimitive().isString()) {
			Type type = pContext.deserialize(pJson.getAsJsonPrimitive(), PolysUnionDeserializer.Type.class);
			return new PolysUnionDeserializer(new ArrayList<>(), type);
		}
		if (pJson.isJsonArray()) {
			...
			return new PolysUnionDeserializer(matrix, null);
		}
